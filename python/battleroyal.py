...
teams = {
    ...
    "iRandomNumberGeneratedThisColour": {
        "colour" : (239, 107, 83),
        "bColour" :(239, 107, 83),
        "spawnX" : -100,
        "spawnAngle" : 0,
    ...
    "actualPitchBlack,likeSoDarkYouCantSeeIt,InOtherWords'KillYourselfDark'": {
        "colour" : (0, 0, 0),
        "bColour" :(0, 0, 0),
        "spawnX" : -100,
        "spawnAngle" : 0,
...
class Bullet:
    def __init__(self, team, x, y, dir):
        self.x = x
        self.y = y
        self.angle = dir + math.radians(random.randint(-20,20))
        self.team = team
        self.timer = 0
        self.speed = 1500
        playSound("sho")
...
class Plane:
    def __init__(self, team):
        ...
        self.bulletCount = random.randint(4, 6)
    ...
    def update(self, dt):
        ...
        if self.targetChangeTimer <= 0:
            ...
            self.targetChangeTimer = random.randint(0, 50)/30 + 1

            #if self.target == planeList.index(self):
            #    self.target = None
            #    self.targetChangeTimer = 0
        ...
        if self.x > window_screen.get_width() + 101:
            self.x = -90
        elif self.x < -101:
            self.x = window_screen.get_width() + 90

        if self.y > window_screen.get_height() + 101:
            self.y = -90
        elif self.y < -101:
            self.y = window_screen.get_height() + 90
...
#                print(str(self.team))
                ...
                if self.bulletCount > 0:
                    ...
                    if self.shootCool < 0:
                        ...
                        spread = teams[self.team]["score"]+1
                        
                        for i in range(0,spread):

                            angleAdd = (((i+1)-((spread+1)/2))/((spread+1)/2))*25

                            bulletList.append(Bullet(self.team, self.x, self.y, self.angle+angleAdd))
    ...
    def draw(self):
        ...
        if len(planeList) == 1:
            scoreText = font.render("Winner: " + str(teams[self.team]["name"]), True, (1,1,1))
...
for i in range(0, 1):
    ...
    planeList.append(Plane("orange"))
    planeList.append(Plane("blue"))
    planeList.append(Plane("green"))
    planeList.append(Plane("grey"))
    planeList.append(Plane("magenta"))
    planeList.append(Plane("cyan"))
    planeList.append(Plane("iKeyboardSpammedthisColour"))
    planeList.append(Plane("iRandomNumberGeneratedThisColour"))
    planeList.append(Plane("chatGPTpickedThisColoursoIdk"))
    planeList.append(Plane("actualPitchBlack,likeSoDarkYouCantSeeIt,InOtherWords'KillYourselfDark'"))
    planeList.append(Plane("orbitechBlue"))
...
while running:
    ...
    bulletList = [bullet for bullet in bulletList if bullet.timer <= 3]
